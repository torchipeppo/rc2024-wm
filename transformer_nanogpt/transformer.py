...
With added support for block-causal mask
"""
...
def block_tril(config):
    causal_mask = torch.tril(torch.ones(config.sequence_size, config.sequence_size))
    block_causal_mask = torch.max(causal_mask, torch.block_diag(*[torch.ones(config.block_size, config.block_size) for _ in range(config.num_of_blocks)]))
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.flash:
            ...
            y = torch.nn.functional.scaled_dot_product_attention(q, k, v, attn_mask=self.attn_mask, dropout_p=self.dropout if self.training else 0)
        else:
...
class Transformer(nn.Module):
    ...
    def __init__(self, config, reserved_tokens):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            wpe = nn.Embedding(config.sequence_size, config.n_embd),
    ...
    def forward(self, idx, targets=None):
        device = idx.device
        b, t = idx.size()
        assert t <= self.config.sequence_size, f"Cannot forward sequence of length {t}, sequence size is only {self.config.sequence_size}"
        ...
        if targets is not None:
            ...
            # inference-time mini-optimization: only forward the lm_head on the very last block
            logits = self.lm_head(x[:, -self.config.block_size:, :])
            loss = None

        return logits, loss

    def crop_sequence_size(self, sequence_size):
        # model surgery to decrease the block size if necessary
        # e.g. we may load the GPT2 pretrained model checkpoint (block size 1024)
        # but want to use a smaller block size for some smaller, simpler model
        assert sequence_size <= self.config.sequence_size
        self.config.sequence_size = sequence_size
        self.transformer.wpe.weight = nn.Parameter(self.transformer.wpe.weight[:sequence_size])
        for block in self.transformer.h:
            if hasattr(block.attn, 'attn_mask'):
                block.attn.attn_mask = block.attn.attn_mask[:,:,:sequence_size,:sequence_size]
    ...
    def estimate_mfu(self, fwdbwd_per_iter, dt):
        ...
        cfg = self.config
        L, H, Q, T = cfg.n_layer, cfg.n_head, cfg.n_embd//cfg.n_head, cfg.sequence_size
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None):
        ...
        for _ in range(max_new_tokens):
            # if the sequence context is growing too long we must crop it at sequence_size
            idx_cond = idx if idx.size(1) <= self.config.sequence_size else idx[:, -self.config.sequence_size:]
...
if __name__ == "__main__":
    breakpoint()   # for free block_tril testing
